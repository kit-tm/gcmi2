...
import static org.projectfloodlight.openflow.protocol.OFVersion.OF_10;
...
implements IFloodlightModule, ILearningSwitchService, IOFMessageListener, IControllerCompletionListener {
	...
	private ConcurrentHashMap<Long, Long> downstreamInMeasurements;
	private ConcurrentHashMap<Long, Long> downstreamOutMeasurements;
	private long lastMessageReceived;
	private OFFactory factory;


	public LearningSwitch(){
		downstreamInMeasurements = new ConcurrentHashMap<>();
		downstreamOutMeasurements = new ConcurrentHashMap<>();
		lastMessageReceived = System.currentTimeMillis();
		//startFileWriteChecker();
		factory = OFFactories.getFactory(OF_10);
	}
	...
	private void writePacketOutForPacketIn(IOFSwitch sw, OFPacketIn packetInMessage, OFPort egressPort) {
		measureTime(packetInMessage.getXid(), false);
	...
	private void startFileWriteChecker(){
		...
		t.scheduleAtFixedRate(new TimerTask() {
			public void run()
			{
				if(System.currentTimeMillis() - lastMessageReceived > 3000 && downstreamInMeasurements.size() + downstreamOutMeasurements.size() > 0){
					writeToFile();
				}
			}
		},10000,3000);
	...
	private void measureTime(long xid, boolean isInputMessage){
		if(isInputMessage){
			//downstreamInMeasurements.put(nativeClock.getCurrentTimeMicros(), xid);
		}else{
			//downstreamOutMeasurements.put(nativeClock.getCurrentTimeMicros(), xid);
		}
	}

	private void writeToFile(){
		final HashMap<Long, Long> downstreamInMeasurements = new HashMap<>(this.downstreamInMeasurements);
		final HashMap<Long, Long> downstreamOutMeasurements = new HashMap<>(this.downstreamOutMeasurements);

		if(this.lock == null){
			this.lock = new ReentrantLock();
		}

		this.downstreamInMeasurements.clear();
		this.downstreamOutMeasurements.clear();

		ArrayList<String> fileContent = new ArrayList<>();

		for(long time : downstreamInMeasurements.keySet()){
			fileContent.add(downstreamInMeasurements.get(time) + " in " + time);
		}

		for(long time : downstreamOutMeasurements.keySet()){
			fileContent.add(downstreamOutMeasurements.get(time) + " out " + time);
		}
	...
	public Command receive(IOFSwitch sw, OFMessage msg, FloodlightContext cntx) {
		...
		case ECHO_REQUEST:
			lastMessageReceived = System.currentTimeMillis();
			measureTime(msg.getXid(), true);
			measureTime(msg.getXid(), false);
